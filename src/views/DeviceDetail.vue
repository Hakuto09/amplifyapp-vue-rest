<template>
  <div :class="$style.component">
    ...
    <br><br>
    <h4>1NCE Latest Devices Positions 情報</h4>
    <h6>(接続先の基地局位置情報)</h6>
    ...
    <br><br>
    <h4>1NCE SIM Data Quata 情報</h4>
    <h6>SIM残量: {{ d_1nce_sim_data_quata_volume }} MB</h6>
    <br><br>
...
const _1nce_access_via_aws = true;
...
//      message_1nce_sim_quota_result: 'こちらは、1NCE OS経由デバイス (via_1nce_os = true) の場合のみ表示されます。',
    ...
    update1nceApiToken: async function() {
      ...
      if (_1nce_access_via_aws) {
        ...
        .then(response => {
          ...
          const _1nce_api_token = response.data.body['access_token'];
  ...
  //      axios.post('https://api.1nce.com/management-api/oauth/token', {
    ...
    update1nceSimDataQuata: async function() {
      ...
      if (_1nce_api_token != '') {
        ...
        const url_path = _1nce_url_base_v + 'sims/' + _1nce_device_id[0] + '/quota/data';
        const headers = {
          'Accept': 'application/json',
        ...
        if (_1nce_access_via_aws) {
          // リクエストの記述
          console.log(fileName, funcName[0], funcName[1], " Before await axios.post(1nce sim quota):", " url_path ", url_path, " headers ", headers);
          const awsIdToken = (await fetchAuthSession()).tokens.idToken ?? '';
          console.log(fileName, funcName[0], funcName[1], "After (await fetchAuthSession()).tokens", ' awsIdToken ', awsIdToken);
          await axios.post(aws_url_base + 'external_get', {
              url: url_path,
              headers: headers,
            }, {
              headers: {
...
//            console.log(fileName, funcName[0], funcName[1], " Before set this.d_1nce_sim_data_quata_all_json:");
            this.d_1nce_sim_data_quata_all_json = response.data.body;
...
//    update1nceLocatePosition: async function() {
    update1nceLocatePosition: async function() {
      ...
      if (_1nce_api_token != '') {
        const _1nce_device_id = this.device_name.split('_');
        const url_path = _1nce_url_base_v + 'locate/positions/latest?deviceId=' + _1nce_device_id[0];
        const headers = {
          'Accept': 'application/json',
          'Authorization': 'Bearer ' + _1nce_api_token,
        };
        ...
        if (_1nce_access_via_aws) {
          // リクエストの記述
          console.log(fileName, funcName[0], funcName[1], " Before await axios.post(1nce locate via aws):", " url_path ", url_path, " headers ", headers);
          const awsIdToken = (await fetchAuthSession()).tokens.idToken ?? '';
          console.log(fileName, funcName[0], funcName[1], "After (await fetchAuthSession()).tokens", ' awsIdToken ', awsIdToken);
          await axios.post(aws_url_base + 'external_get', {
              url: url_path,
          ...
          .then(response => {
            ...
            if (response.data.body.coordinates == []) {
  ...
  //            console.log(fileName, funcName[0], funcName[1], " Before set map position");
              this.d_1nce_map_marker_position = [response.data.body.coordinates[0].coordinate[1], response.data.body.coordinates[0].coordinate[0]]; // 緯度 latitude, 経度 longitude
